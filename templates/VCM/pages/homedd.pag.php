...
<?php } ?>
	<?php }if(constant("Show_Warning") == true){ ?>
        	{%SHOW_WARNING%}
        <?php }if(constant("L_Show_MGuild") == true){ ?>
        <div class="box">
        	<div class="header">&raquo; Melhor Guild</div>
            <div id="mguild">
			<?php
			global $CTM_MSSQL;
            $qg = $CTM_MSSQL->query("SELECT TOP 1 G_Name,G_Mark,G_Score FROM ".MuGen_DB.".dbo.Guild ORDER BY G_Score DESC");			
            $guild = mssql_fetch_array($qg);
            echo("<p id=\"mmguild\">Guild: 
			<a href=\"javascript: void(EffectWeb);\" onclick=\"CTM_Load('?pag=search&guild=".urlencode($guild[0])."','conteudo','GET');\">".$guild[0]."</a></p>");
            echo ("<p id=\"smguild\">Score: <span class=\"orange\">".$guild[2]." Score</span></p>");
			echo("<img src=\"?public=logoGuild&amp;code=".urlencode(bin2hex($guild[1]))."\" width=\"70\" height=\"70\" alt=\"\">");
            ?>         
            </div>
        </div>
        <div class="box">
        	<div class="header">&bull; Rankings</div>
                ...
                <div id="all-tops">
                    <ul class="rank">
						...
						<?php
                            global $CTM_MSSQL;
                            $query = $CTM_MSSQL->query("SELECT TOP 1 Name,".Resets.",CTM_Image FROM ".MuGen_DB.".dbo.Character ORDER BY ".Resets." DESC");
                            $cont = NULL;
                ...
                <div id="all-tops">
                    <ul class="rank">
						...
						<?php
                            global $CTM_MSSQL;
                            $query = $CTM_MSSQL->query("SELECT TOP 1 Name,".mresets.",CTM_Image FROM ".MuGen_DB.".dbo.Character ORDER BY ".mresets." DESC");
                            $cont = NULL;
							while($topresets = mssql_fetch_array($query)){
							$cont++;
							if($topresets[2] == NULL OR empty($topresets[2]) OR !file_exists(Upload_Img."".$topresets[2]."")){
                    ...
                    <ul class="rank">
						...
						<?php
                            global $CTM_MSSQL;
                            $query = $CTM_MSSQL->query("SELECT TOP 1 Name,".horastotal.",CTM_Image FROM ".MuGen_DB.".dbo.Character ORDER BY ".horastotal." DESC");
                            $cont = NULL;
							while($topresets = mssql_fetch_array($query)){
							$cont++;
							if($topresets[2] == NULL OR empty($topresets[2]) OR !file_exists(Upload_Img."".$topresets[2]."")){
                    ...
                    <ul class="rank">
    					<li>
                            <span class="title-r">Top Pk</span>
								<?php
                                global $CTM_MSSQL;
                                $query = $CTM_MSSQL->query("SELECT TOP 1 Name,".PkRank.",CTM_Image FROM ".MuGen_DB.".dbo.Character ORDER BY ".PkRank." DESC");
								$cont = NULL;
                                	while($topresets = mssql_fetch_array($query)){
                                	$cont++;
                                    if($topresets[2] == NULL OR empty($topresets[2]) OR !file_exists(Upload_Img."".$topresets[2]."")){
                    ...
                    <ul class="rank">
    					<li>
                            <span class="title-r">Top Her&oacute;i</span>
								<?php
                                global $CTM_MSSQL;
                                $query = $CTM_MSSQL->query("SELECT TOP 1 Name,".RankingHeroi.",CTM_Image FROM ".MuGen_DB.".dbo.Character ORDER BY ".RankingHeroi." ASC");
								$cont = NULL;
                ...
                <div id="day-tops">
                    <ul class="rank">
						...
						<?php
                            global $CTM_MSSQL;
                            $query = $CTM_MSSQL->query("SELECT TOP 1 Name,".resetsDay.",CTM_Image FROM ".MuGen_DB.".dbo.Character ORDER BY ".resetsDay." DESC");
                            $cont = NULL;
							while($topresets = mssql_fetch_array($query)){
							$cont++;
							if($topresets[2] == NULL OR empty($topresets[2]) OR !file_exists(Upload_Img."".$topresets[2]."")){
                ...
                <div id="day-tops">
                    <ul class="rank">
						...
						<?php
                            global $CTM_MSSQL;
                            $query = $CTM_MSSQL->query("SELECT TOP 1 Name,".mrdiario.",CTM_Image FROM ".MuGen_DB.".dbo.Character ORDER BY ".mrdiario." DESC");
                            $cont = NULL;
							while($topresets = mssql_fetch_array($query)){
							$cont++;
							if($topresets[2] == NULL OR empty($topresets[2]) OR !file_exists(Upload_Img."".$topresets[2]."")){
                    ...
                    <ul class="rank">
						...
						<?php
                            global $CTM_MSSQL;
                            $query = $CTM_MSSQL->query("SELECT TOP 1 Name,".horasdiario.",CTM_Image FROM ".MuGen_DB.".dbo.Character ORDER BY ".horasdiario." DESC");
                            $cont = NULL;
							while($topresets = mssql_fetch_array($query)){
							$cont++;
							if($topresets[2] == NULL OR empty($topresets[2]) OR !file_exists(Upload_Img."".$topresets[2]."")){
                    ...
                    <ul class="rank">
    					<li>
                            <span class="title-r">Top Pk</span>
								<?php
                                global $CTM_MSSQL;
                                $query = $CTM_MSSQL->query("SELECT TOP 1 Name,".pkdiario.",CTM_Image FROM ".MuGen_DB.".dbo.Character ORDER BY ".pkdiario." DESC");
								$cont = NULL;
                                	while($topresets = mssql_fetch_array($query)){
                                	$cont++;
                                    if($topresets[2] == NULL OR empty($topresets[2]) OR !file_exists(Upload_Img."".$topresets[2]."")){
                    ...
                    <ul class="rank">
    					<li>
                            <span class="title-r">Top Her&oacute;i</span>
								<?php
                                global $CTM_MSSQL;
                                $query = $CTM_MSSQL->query("SELECT TOP 1 Name,".RankingHeroi_Diario.",CTM_Image FROM ".MuGen_DB.".dbo.Character ORDER BY ".RankingHeroi_Diario." ASC");
								$cont = NULL;
                                	while($topweek = mssql_fetch_array($query)){
                                	$cont++;
                                    if($topweek[2] == NULL OR empty($topweek[2]) OR !file_exists(Upload_Img."".$topweek[2]."")){
...
<center> <A href="http://www.mumonster.net/website/premiacao/index.php" title="Premiação Ranking "><b><font color="#FF0000">[Ver Premiação]</font></b></A>  </center>
                ...
                <div id="week-tops">
					<ul class="rank">
						...
						<?php
                            global $CTM_MSSQL;
                            $query = $CTM_MSSQL->query("SELECT TOP 1 Name,".resetsWeek.",CTM_Image FROM ".MuGen_DB.".dbo.Character ORDER BY ".resetsWeek." DESC");
                            $cont = NULL;
							while($topresets = mssql_fetch_array($query)){
							$cont++;
							if($topresets[2] == NULL OR empty($topresets[2]) OR !file_exists(Upload_Img."".$topresets[2]."")){
                ...
                <div id="day-tops">
                    <ul class="rank">
						...
						<?php
                            global $CTM_MSSQL;
                            $query = $CTM_MSSQL->query("SELECT TOP 1 Name,".mrsemanal.",CTM_Image FROM ".MuGen_DB.".dbo.Character ORDER BY ".mrsemanal." DESC");
                            $cont = NULL;
							while($topresets = mssql_fetch_array($query)){
							$cont++;
							if($topresets[2] == NULL OR empty($topresets[2]) OR !file_exists(Upload_Img."".$topresets[2]."")){
                    ...
                    <ul class="rank">
						...
						<?php
                            global $CTM_MSSQL;
                            $query = $CTM_MSSQL->query("SELECT TOP 1 Name,".horassemanal.",CTM_Image FROM ".MuGen_DB.".dbo.Character ORDER BY ".horassemanal." DESC");
                            $cont = NULL;
							while($topresets = mssql_fetch_array($query)){
							$cont++;
							if($topresets[2] == NULL OR empty($topresets[2]) OR !file_exists(Upload_Img."".$topresets[2]."")){
                    ...
                    <ul class="rank">
    					<li>
                            <span class="title-r">Top Pk</span>
								<?php
                                global $CTM_MSSQL;
                                $query = $CTM_MSSQL->query("SELECT TOP 1 Name,".pksemanal.",CTM_Image FROM ".MuGen_DB.".dbo.Character ORDER BY ".pksemanal." DESC");
								$cont = NULL;
                                	while($topresets = mssql_fetch_array($query)){
                                	$cont++;
                                    if($topresets[2] == NULL OR empty($topresets[2]) OR !file_exists(Upload_Img."".$topresets[2]."")){
                    ...
                    <ul class="rank">
    					<li>
                            <span class="title-r">Top Her&oacute;i</span>
								<?php
                                global $CTM_MSSQL;
                                $query = $CTM_MSSQL->query("SELECT TOP 1 Name,".RankingHeroi_Semanal.",CTM_Image FROM ".MuGen_DB.".dbo.Character ORDER BY ".RankingHeroi_Semanal." ASC");
								$cont = NULL;
                                	while($topweek = mssql_fetch_array($query)){
                                	$cont++;
                                    if($topweek[2] == NULL OR empty($topweek[2]) OR !file_exists(Upload_Img."".$topweek[2]."")){
...
<center> <A href="http://www.mumonster.net/website/premiacao/index.php" title="Premiação Ranking "><b><font color="#FF0000">[Ver Premiação]</font></b></A>  </center>
                ...
                <div id="trofeu-tops">
					<?php
                        global $CTM_MSSQL;
                        $query = $CTM_MSSQL->query("SELECT TOP 5 Name,".medalha.",CTM_Image FROM ".MuGen_DB.".dbo.Character ORDER BY ".medalha." DESC");
						$cont = NULL;
                            while($topresets = mssql_fetch_array($query)){
                            $cont++;
                            if($topresets[2] == NULL OR empty($topresets[2]) OR !file_exists(Upload_Img."".$topresets[2]."")){
                            ...
                            echo '<ul class="rank">
    							<li>
								<ul><a href="javascript: void(EffectWeb);" onclick="CTM_Load(\'?pag=search&amp;char='.$topresets[0].'\', \'conteudo\',\'GET\');">
								<li>'.$topresets[0].'</li>
								<li><img src="'.$photo.'" width="90" height="90" alt="Imagem de '.$topresets[0].'"></li>
                                <li>( '.$topresets[1].' Medalhas )</li>
                                </a></ul>
								</li>
                 		   </ul>';
                            }
                ...
                <div id="medalha-tops">
					<?php
                        global $CTM_MSSQL;
                        $query = $CTM_MSSQL->query("SELECT TOP 5 Name,".trofeu.",CTM_Image FROM ".MuGen_DB.".dbo.Character ORDER BY ".trofeu." DESC");
						$cont = NULL;
                            while($topresets = mssql_fetch_array($query)){
                            $cont++;
                            if($topresets[2] == NULL OR empty($topresets[2]) OR !file_exists(Upload_Img."".$topresets[2]."")){
			...
			<script type="text/javascript">
				...
				aba.addTab("trofeu-tops", "Medalha");
				aba.addTab("medalha-tops", "Trofeu");
			</script>
            </div>
